...
import Input from "@blend-ui/core/dist/esm/Input";
const axios = require("axios");
// import ToastProvider from "@blend-ui/toast/dist/esm/ToastProvider";
import { Toast } from "@blend-ui/toast/dist/esm/Toast";
...
import { Box, Flex, Text, Button, useTheme, TextArea } from "@blend-ui/core";
...
const Content = forwardRef((props, ref) => {
  const { appSyncClient, prifinaID, updateDebug, appSettings } = props;
  const url = useState("a")
  // const {
  //   check,
  //   currentUser,
  //   getLocalization,
...
*/
  ...
  */
  ...
  const handleCopyClick = (text) => {
    ...
    copyTextToClipboard(text)
      .then(() => {
        ...
        setIsCopied(true);
        console.log(toast)
        toast.info("App ID copied to clipboard", {});
        setTimeout(() => {
          setIsCopied(false);
        }, 1500);
      })
  ...
  "variables": {"id": props.GcmsID}}
  const updateApp = {
    "operationName":"updateApp",
    "query": `
    mutation updateApp($id: ID, $remoteUrl: String) {
      updateApp(data: {remoteUrl: $remoteUrl}, where: {id: $id}){
        id
      }
    }`,
  "variables": {"id": props.GcmsID, "remoteUrl": dataTemp.remoteUrl}}
  ...
  const fetchDetails = async e => {
    try {
      const repsonse = await axios({
        ...
        method: "post",
        headers: headers,
        data: getDetailsQuery,
      });
      console.log(repsonse);
      setData(repsonse.data.data.app);
      setDataTemp({
        appId: repsonse.data.data.app.appId,
        remoteUrl: repsonse.data.data.app.remoteUrl,
        settings: JSON.stringify(repsonse.data.data.app.settings)
  ...
  const saveJSON = async e => {
    ...
    const updateAppSettings = {
      "operationName":"updateApp",
      "query": `
      mutation updateApp($id: ID, $settings: [Json!]) {
        updateApp(data: {settings: $settings}, where: {id: $id}){
          id
        }
      }`,
    "variables": {"id": props.GcmsID, "settings": JSON.parse(dataTemp.settings)}}
      ...
      if (repsonse.status===200){
        ...
        setData({
          ...data,
          settings: JSON.parse(dataTemp.settings)
        })
      ...
      console.log("BAD REQUEST - Please make sure data is in correct JSON form. See: https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/JSON/parse for more details")
    }
  };

  const [step, setStep] = useState(0)
  const [parseSettings, setParseSettings] = useState(false)
  ...
  return (
    <>
        <React.Suspense fallback={"Loading ..."}>
          ...
          <Text fontSize="sm" style={{marginRight: '.5rem'}}>{props.name}</Text>
          <Text fontSize="sm" style={{border: "2px solid blue",
          borderRadius: "16px", 
          paddingLeft: "10px", paddingRight: "10px", color: "blue"}}>{props.type}</Text>
        ...
        {data.remoteUrl===null||data.remoteUrl===""? (
          ...
          marginTop="auto">
            ...
            <Text style={{margin: "auto"}}>Empty State</Text>
            </>
          </Flex>
        ...
        //     paddingTop="1rem"
        //     flexDirection="row">
        //   <Flex alignItems="center" mr="20px" height="fit-content">
        //     <BlendIcon
        ...
        //     />
        //   </Flex>
        //   <Text fontSize="sm" style={{marginRight: '1rem'}}>Test Settings</Text>
        //   <Text fontSize="sm" style={{marginRight: '1rem'}}>Widget Settings</Text>
        //   <Text fontSize="sm" style={{marginRight: '1rem'}}>Debugging</Text>
        //   <Text fontSize="sm" style={{marginRight: '1rem'}}>Support</Text>
        ...
        // <Flex
        // >
        // <Text fontSize="lg" style={{marginRight: '1rem'}}>Test Settings</Text>
        // </Flex>
        <Flex
        ...
        marginBottom="25px"
        >
          <Flex alignItems="center" mr="20px" height="fit-content" marginBottom="10px">
             <BlendIcon
              iconify={mdiArrowCollapse}
              width="24px"
              onClick={()=>{setExpand(false)}}
            />
          </Flex>
          {step === 0 ? (
           <Text fontSize="sm" style={{marginRight: '1rem', fontWeight: "bold", borderBottom: "3px solid blue"}}>Test Settings</Text>
          ): (
            <Text fontSize="sm" style={{marginRight: '1rem', borderBottom: "3px solid grey"}} onClick={()=>{setStep(0)}}>Test Settings</Text>
          )}
          {step === 1 ? (
           <Text fontSize="sm" style={{marginRight: '1rem', fontWeight: "bold", borderBottom: "3px solid blue"}}>Widget Settings</Text>
          ): (
            <Text fontSize="sm" style={{marginRight: '1rem', borderBottom: "3px solid grey"}} onClick={()=>{setStep(1)}}>Widget Settings</Text>
          )}
          {step === 2 ? (
           <Text fontSize="sm" style={{marginRight: '1rem', fontWeight: "bold", borderBottom: "3px solid blue"}}>Debugging</Text>
          ): (
            <Text fontSize="sm" style={{marginRight: '1rem', borderBottom: "3px solid grey"}} onClick={()=>{setStep(2)}}>Debugging</Text>
          )}
          {step === 3 ? (
           <Text fontSize="sm" style={{marginRight: '1rem', fontWeight: "bold", borderBottom: "3px solid blue"}}>Support</Text>
          ): (
            <Text fontSize="sm" style={{marginRight: '1rem', borderBottom: "3px solid grey"}} onClick={()=>{setStep(3)}}>Support</Text>
          )}
        </Flex>
            ...
            {step === 0  && (
                      ...
                      <Text fontSize="lg" style={{marginLeft: '1rem', marginBottom:"10px"}}>Test Session Setup</Text>
                      <Text fontSize="lg" style={{marginLeft: '1rem'}}>App ID</Text>
                ...
                <Input
                  ...
                  value={props.id}
                  color="black"
                  disabled /><Flex alignItems="center" mr="20px" height="fit-content">
                  <BlendIcon
                    iconify={mdiContentCopy}
                    width="24px"
                    onClick={() => { handleCopyClick(props.id); } } />
                </Flex>
              </Flex><Text fontSize="lg" style={{ marginLeft: '1rem' }}>Remote Build Link</Text><Input
                  width="684px"
                  label="text"
                  value={dataTemp.remoteUrl}
                  onChange={handleInputChange}
                  color="black" 
                  marginBottom="10px"/><Button style={{ width: "fit-content" }} onClick={() => { saveRemoteBuild(); } }>Refresh</Button>
                  </Flex>
                  <Flex
                  bg="#D3D3D3"
                  flexDirection="row"
                  ...
                  >
                    ...
                    <Text fontSize="lg">Set up test session</Text>
                    <Text>1. Copy your app ID</Text>
                    <Text>2. Add the app ID to your local build</Text>
                    <Text>3. Copy the remote link for your local build</Text>
                    <Text>4. Add the link to the sandbox launcher</Text>
                    <Text>5. Hit refresh</Text>
                    </Flex>
                    <Flex
                    bg="black"
                    width="50px"
            ...
            {step === 1  && (
                      ...
                      <Flex flexDirection="column" marginLeft="20px" marginRight="20px" width="100%" height="100%">
                      <Text fontSize="lg" style={{marginLeft: '1rem', marginBottom:"10px"}}>Widget Settings</Text>
                ...
                <TextArea
                  ...
                  height="100%"
                  name={"settings"}
                  placeholder='Example: [{"name":"test1","values":[1,2,3]},{"name":"test2","values":[5,10,15]}]'
                  value={dataTemp.settings}
                  onChange={handleInputChange}
                ...
                />
                  ...
                  {JSON.stringify(data.settings) === dataTemp.settings ? (
                    <>
                    <Button style={{ width: "fit-content" }} onClick={saveJSON} disabled>Submit</Button>  
                    </>
                  ): (
                    <>
                    <Button style={{ width: "fit-content" }} onClick={saveJSON}>Submit</Button>  
                    </>
                  )}
